...
Troop Maps::Tiles::QuantityTroop() const
{
    return MP2::isCaptureObject(GetObject(false))
               ? world.GetCapturedObject(GetIndex()).GetTroop()
               : Troop(QuantityMonster(), MonsterCount());
}
...
void Maps::Tiles::PlaceMonsterOnTile(Tiles& tile, const Monster& mons, uint32_t count)
{
...
void Maps::Tiles::UpdateDwellingPopulation(Tiles& tile)
{
    ...
    case MP2::OBJ_CITYDEAD:
        count = 1 < world.CountWeek() &&
                Color::NONE == tile.QuantityColor()
                    ? 0
                    : troop().GetRNDSize(true);
        break;
