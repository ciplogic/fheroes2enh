...
public:
            ...
            tradpost(Settings::Get().ExtGameEvilInterface() ? ICN::TRADPOSE : ICN::TRADPOST)
    {
        ...
        TextBox(_("Please inspect our fine wares. If you feel like offering a trade, click on the items you wish to trade with and for."),
                                   ...
                                   uint32_t count_sell, bool fromTradingPost)
{
    ...
    if (disable || resourceFrom == resourceTo || Resource::GOLD != resourceTo && 0 == max_buy)
    {
        ...
        TextBox(_("You have received quite a bargain. I expect to make no profit on the deal. Can I interest you in any of my other wares?"),
                Font::BIG, dst_rt);
    ...
    } else
    {
        ...
        RedrawInfoBuySell(count_sell, count_buy, max_sell,
                          world.GetKingdom(Settings::Get().CurrentColor()).GetFunds().Get(resourceTo));
        splitter.ShowCursor();
        cursor.Show();
        display.Flip();
    }
    ...
    while (le.HandleEvents())
    {
        ...
        for (uint32_t ii = 0; ii < rectsFrom.size(); ++ii)
        {
            ...
            if (le.MouseClickLeft(rect_from))
            {
                ...
                if (resourceTo)
                    gui.ShowTradeArea(resourceFrom, resourceTo, max_buy, max_sell, count_buy, count_sell,
                                      fromTradingPost);
        ...
        if (buttonLeft.isEnable() && buttonRight.isEnable() && max_buy && le.MousePressLeft(splitter.GetRect()))
        {
            ...
            count_buy = seek *
                        (Resource::GOLD == resourceTo ? GetTradeCosts(resourceFrom, resourceTo, fromTradingPost) : 1);
            count_sell = seek *
                         (Resource::GOLD == resourceTo ? 1 : GetTradeCosts(resourceFrom, resourceTo, fromTradingPost));
        ...
        if (buttonMax.w && max_buy && le.MouseClickLeft(buttonMax))
        {
            ...
            count_sell =
                    max * (Resource::GOLD == resourceTo ? 1 : GetTradeCosts(resourceFrom, resourceTo, fromTradingPost));
        ...
        if (buttonMin.w && max_buy && le.MouseClickLeft(buttonMin))
        {
            ...
            count_sell =
                    min * (Resource::GOLD == resourceTo ? 1 : GetTradeCosts(resourceFrom, resourceTo, fromTradingPost));
...
void RedrawToResource(const Point &pt, bool showcost, bool tradingPost, int from_resource)
{
    ...
    // mercury
    RedrawResourceSprite2(AGG::GetICN(tradpost, 8), pt.x + 37, pt.y, showcost, from_resource, Resource::MERCURY,
                          tradingPost);
    // ore
    RedrawResourceSprite2(AGG::GetICN(tradpost, 9), pt.x + 74, pt.y, showcost, from_resource, Resource::ORE,
                          tradingPost);
    // sulfur
    RedrawResourceSprite2(AGG::GetICN(tradpost, 10), pt.x, pt.y + 37, showcost, from_resource, Resource::SULFUR,
                          tradingPost);
    // crystal
    RedrawResourceSprite2(AGG::GetICN(tradpost, 11), pt.x + 37, pt.y + 37, showcost, from_resource, Resource::CRYSTAL,
                          tradingPost);
    // gems
    RedrawResourceSprite2(AGG::GetICN(tradpost, 12), pt.x + 74, pt.y + 37, showcost, from_resource, Resource::GEMS,
                          tradingPost);
    // gold
    RedrawResourceSprite2(AGG::GetICN(tradpost, 13), pt.x + 37, pt.y + 74, showcost, from_resource, Resource::GOLD,
                          tradingPost);
}
