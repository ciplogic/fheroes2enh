...
int main(int argc, char **argv)
{
    ...
    for (int ii = 0; ii < count_sprite; ++ii)
    {
        ...
        u32 data_size = (ii + 1 != count_sprite ? headers[ii + 1].offsetData - head.offsetData : total_size -
                                                                                                 head.offsetData);
        ...
        if (buf.size())
        {
            ...
            fs << " <sprite index=\"" << ii + 1 << "\" name=\"" << shortdstfile.c_str() << "\" ox=\"" << head.offsetX
               << "\" oy=\"" << head.offsetY << "\"/>" << std::endl;
        }
    }
