...
vector<u8> BinaryFileReader::getRaw(u32 size)
{
...
StreamBuf BinaryFileReader::toStreamBuf(u32 size)
{
	StreamBuf sb;
	vector<u8> buf = getRaw(size);
	sb.putRaw(reinterpret_cast<const char *>(&buf[0]), size);
	return sb;
}
